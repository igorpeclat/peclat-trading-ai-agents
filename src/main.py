...
def run_script(script: str, passthrough_args: list[str]) -> int:
    scripts = {
        "backtests-admin": "src/scripts/backtests_admin_tui.py",
    }
    if script not in scripts:
        cprint(f"❌ Unknown script: {script}", "red")
        return 2
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Moon Dev AI Trading System")
    parser.add_argument(
        "--script",
        choices=["backtests-admin"],
        help="Run utility script instead of agent loop",
    )
    parser.add_argument(
        "script_args",
        nargs=argparse.REMAINDER,
        help="Arguments passed to --script target (prefix with --)",
    )
...
if __name__ == "__main__":
    args = parse_args()
    if args.script:
        forward = args.script_args
        if forward and forward[0] == "--":
            forward = forward[1:]
        raise SystemExit(run_script(args.script, forward))
