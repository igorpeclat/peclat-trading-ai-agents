...
class WhaleAgent(BaseAgent):
        ...
        try:
            ...
            if self.history_file.exists():
                ...
                if all(col in df.columns for col in required_columns):
                    ...
                    self.oi_history = df
                    self.oi_history["timestamp"] = pd.to_datetime(
                        self.oi_history["timestamp"]
                    )
            ...
            if not self.oi_history.empty:
                ...
                self.oi_history = self.oi_history[
                    self.oi_history["timestamp"] > cutoff_time
                ]
    ...
    def _save_oi_data(self, timestamp, btc_oi, eth_oi, total_oi):
        ...
        try:
            ...
            if not self.oi_history.empty:
                ...
                btc_change_pct = (
                    (btc_oi - prev_data["btc_oi"]) / prev_data["btc_oi"]
                ) * 100
                eth_change_pct = (
                    (eth_oi - prev_data["eth_oi"]) / prev_data["eth_oi"]
                ) * 100
                total_change_pct = (
    ...
    def _calculate_changes(self, current_oi):
        ...
        print("=" * 80)
        print(
            "Timestamp | BTC OI | ETH OI | Total OI | BTC Change% | ETH Change% | Total Change%"
        )
        print("-" * 80)
        ...
        try:
            ...
            if not changes:
                ...
                for period in available_periods:
                    ...
                    message += (
                        f"OI is {direction} {abs(change):.1f}% over the last {period}. "
                    )
    ...
    def _detect_whale_activity(self, current_change):
        ...
        try:
            ...
            historical_changes = (
                self.oi_history["btc_change_pct"]
                .abs()
                .rolling(window=10)
                .mean()
                .dropna()
