...
## 🧰 Backtest Admin Runner

- Trade-padre style admin flow is available via:
  - `python src/scripts/backtests_admin_tui.py`
- Run one-off without interactive TUI:
  - `python src/scripts/backtests_admin_tui.py --no-tui --run-latest`
  - `python src/scripts/backtests_admin_tui.py --no-tui --file src/agents/test_execution/test_backtest_v2.py`
- You can also launch through main entrypoint:
  - `python src/main.py --script backtests-admin -- --no-tui --run-latest`
- Results are saved to:
  - `src/data/execution_results/*.json`
